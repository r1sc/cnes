...
void read_ines(const char* path, ines_t* ines) {
	...
	size_t prg_rom_size = 16384 * header.prg_rom_16k_chunks;
	ines->prg_rom = (uint8_t*)malloc(prg_rom_size);
	if (!ines->prg_rom) exit(1);
	fread(ines->prg_rom, prg_rom_size, 1, f);

	size_t chr_rom_size = 8192 * header.chr_rom_8k_chunks;
	ines->chr_rom = (uint8_t*)malloc(chr_rom_size);
	if (!ines->chr_rom) exit(1);
	fread(ines->chr_rom, chr_rom_size, 1, f);
